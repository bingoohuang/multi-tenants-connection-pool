...
import static com.github.bingoohuang.mtcp.pool.TestElf.getPool;
...
public class ConnectionPoolSizeVsThreadsTest {
   ...
   private Counts testPoolSize(final int minIdle, final int maxPoolSize, final int threadCount,
      ...
      log.info("Starting test (minIdle={}, maxPoolSize={}, threadCount={}, workTimeMs={}, restTimeMs={}, connectionAcquisitionTimeMs={}, iterations={}, postTestTimeMs={})",
      ...
      try (final LightDataSource ds = new LightDataSource(config)) {
         ...
         final LightPool pool = getPool(ds);
         ...
         log.info("Test Over, waiting for post delay time {}ms ", postTestTimeMs);
         ...
         if (ref.get() != null) {
            log.error("Task failed", ref.get());
            fail("Task failed");
         }

         log.info("Under Load... {}", underLoad);
         log.info("Post Load.... {}", postLoad);
   ...
   private static class Counts {
      ...
      void updateMaxCounts(final LightPool pool) {
