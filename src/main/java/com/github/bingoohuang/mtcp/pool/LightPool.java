...
public final class LightPool extends PoolBase implements LightPoolMXBean, ConcurrentBag.IBagStateListener {
   ...
   public int getActiveConnections() {
      return connectionBag.getCount(ConcurrentBag.IConcurrentBagEntry.STATE_USING);
   }
   ...
   public int getIdleConnections() {
      return connectionBag.getCount(ConcurrentBag.IConcurrentBagEntry.STATE_FREE);
   }
   ...
   private void abortActiveConnections(final ExecutorService assassinExecutor) {
      for (val poolEntry : connectionBag.values(ConcurrentBag.IConcurrentBagEntry.STATE_USING)) {
         val connection = poolEntry.close();
         try {
      ...
      public void run() {
         try {
            ...
            if (idleTimeout > 0L && config.getMinimumIdle() < config.getMaximumPoolSize()) {
               ...
               val notInUse = connectionBag.values(ConcurrentBag.IConcurrentBagEntry.STATE_FREE);
