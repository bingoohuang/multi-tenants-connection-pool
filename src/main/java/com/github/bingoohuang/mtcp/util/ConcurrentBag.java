...
import static com.github.bingoohuang.mtcp.util.ConcurrentBag.IConcurrentBagEntry.*;
...
public class ConcurrentBag<T extends ConcurrentBag.IConcurrentBagEntry> implements AutoCloseable {
   ...
   public T borrow(long timeout, final TimeUnit timeUnit) throws InterruptedException {
      ...
      try {
         for (T bagEntry : sharedList) {
            if (bagEntry.compareAndSet(STATE_FREE, STATE_USING)) {
         ...
         do {
            ...
            if (bagEntry == null || bagEntry.compareAndSet(STATE_FREE, STATE_USING)) {
               return bagEntry;
            }
   ...
   public void requite(final T bagEntry) {
      bagEntry.setState(STATE_FREE);

      for (int i = 0; waiters.get() > 0; i++) {
         if (bagEntry.getState() != STATE_FREE || handoffQueue.offer(bagEntry)) {
            return;
         } else if ((i & 0xff) == 0xff) {
   ...
   public boolean reserve(final T bagEntry) {
      return bagEntry.compareAndSet(STATE_FREE, STATE_RESERVED);
   }
   ...
   public void unreserve(final T bagEntry) {
      if (bagEntry.compareAndSet(STATE_RESERVED, STATE_FREE)) {
         // spin until a thread takes it or none are waiting
         while (waiters.get() > 0 && !handoffQueue.offer(bagEntry)) {
            yield();
         }
